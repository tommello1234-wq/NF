...
export function gerarQrCodeNfce(input: QrCodeInput): QrCodeOutput {
  ...
  // CSC ID sem zero-padding (alguns autorizadores rejeitam '000001' e aceitam '1')
  const cscId = String(input.cscId).replace(/^0+/, '') || '0'

  // Stringão pra calcular o hash. Pelo Manual NFC-e v7.00 §7.5: concatenação
  // direta sem separadores. A URL final usa | entre campos, mas o hash é só
  // dos valores concatenados + o token.
